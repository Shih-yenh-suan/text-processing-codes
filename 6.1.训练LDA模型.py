...
def train_lda(data_folder, excel_path, num_topics_range, num_iterations):
    ...
    for num_topics in num_topics_range:
        # Train LDA model
        lda = LdaModel(corpus, num_topics=num_topics, id2word=dictionary, iterations=num_iterations)

        # Calculate perplexity
        calc_perplexity = perplexity(lda, test_corpus, dictionary, len(dictionary.keys()), num_topics)
...
def main():
    ...
    step = 2
    data_folder = "csrReport5.1_removeMostCommonWord"
    excel_path = f"csrReport6.1_{start}_{end}_{step}.xlsx"
